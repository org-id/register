...
def main():
    with open('url_testing.csv', 'w') as outputfile:
        ...
        csvwriter.writerow(['List', 'Field', 'List_confirmed?', 'URL_Status', 'URL'])
            ...
            for reglistfile in files:
                ...
                if p.match(reglistfile):
                    ...
                    if reglist['url'] and reglist['access']['publicDatabase']:
                        ...
                        csvwriter.writerow([reglist['code'], 'url', reglist['confirmed'], status, url])
                        url, status = url_status(urls['database_url'])
                        csvwriter.writerow([reglist['code'], 'access.publicDatabase', reglist['confirmed'], status, url])
                    elif reglist['url']:
                        urls = {'list_url': reglist['url']}
                        url, status = url_status(urls['list_url'])
                        csvwriter.writerow([reglist['code'], 'url', reglist['confirmed'], status, url])
                    else:
                        csvwriter.writerow([reglist['code'], 'url', reglist['confirmed'], 'NO URL', ''])
